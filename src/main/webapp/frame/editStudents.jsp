<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="/struts-tags" prefix="s" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head></head>
<body>
<table border="0" width="600px">
<tr>
<td align="center" style="font-size:24px; color:#666"> 学生添加</td>
</tr>
<tr>
<td align="right" > 
<a href="javascript:document.getElementById('saveForm').submit()">保存</a> &nbsp;&nbsp;
<a href="javascript:history.go(-1)">退回</a>
</td>
</tr>
</table>
<br/>

<s:form action="students_update" method="post" namespace="/" id="saveForm" theme="simple">
<s:hidden name="sid" value="%{model.sid}" />
<table border='0' cellpadding="0"  cellspacing="10">

<tr>

<td>学生姓名：</td>
<td><s:textfield name="sname" value="%{model.sname}"/></td>
<td>性别：</td>
<td><s:radio name="sgender" list="{'男','女'}" value="%{model.sgender}"/></td>
</tr>

<tr>
<td>用户名：</td>
<td><s:textfield name="username" value="%{model.username}" /></td>
<td>密码：</td>
<td><s:password name="password" value="%{model.password}" showPassword="true"/></td>
</tr>
<tr>

<td>出生日期：</td>
<td><input type="text" name="birthday" value="<s:date name="model.birthday" format="yyyy-MM-dd" />" /></td>

<td>学号：</td>
<td><s:textfield name="sno" value="%{model.sno}"/></td>
</tr>



<tr>
<td>所属专业班级：</td>
<td><s:select name="classroom.cid" list="list" value="%{model.classroom.cname}" listKey="cid" listValue="cname" headerKey="" headerValue="---请选择---"/></td>
</tr>
</table>
</s:form>
</body>
</html>